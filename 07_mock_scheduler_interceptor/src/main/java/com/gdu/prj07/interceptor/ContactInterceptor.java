 ...
 *                            ㄴ인터셉트가 언제 동작하는지 작성하는 곳
 * 
 * filter : 중간 다리 역할(요청을 막을 수 없다)
 * interceptor : 특정 조건으로 요청을 막을 수 있다.
 * 3. 생성 방법
 *    1) HandlerInterceptor 인터페이스 구현 (권장)
 *    2) HandlerInterceptorAdaptor 클래스 상속
 * 4. 주요 메소드
 *    1) preHandle()       : 요청 이전에 동작할 코드 (요청을 막을 수 있다.), boolean 반환
...
public class ContactInterceptor implements HandlerInterceptor {
  ...
  public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
    ...
    out.println("history.back()");    // 요청을 막는 것을 수행
    out.println("</script>");
