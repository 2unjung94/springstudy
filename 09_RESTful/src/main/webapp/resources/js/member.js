...
const fnGetMemberByNo = (evt)=>{
  $.ajax({
    ...
    if(resData.member !== null){
      ...
      jqEmail.val(resData.member.email).prop('disabled', true);		// 이메일 못바꾸게 diabled 처리함
...
const fnModifyMember = ()=>{
	$.ajax({
		type: 'PUT',
		url: fnGetContextPath() + '/members',
		contentType: 'application/json',		// java 측에서 알아야 하므로 json 이 아닌 application/json
		data: JSON.stringify({
...
const fnRemoveMembers = ()=>{
	// 체크된 요소를 배열에 저장하기
	let arr = [];
	// jQuery 배열로 저장된 값을 꺼내면 더이상 jQuery 가 아닌 js 가 된다
	$.each( $('.chk-member'), (i, chk)=>{
	  if($(chk).is(':checked')){
		  arr.push(chk.value);				// 선택한 memberNo 를 arr 에 저장 (js 문법)
	  }		
	})
	...
	$.ajax({
		type: 'DELETE',
		url: fnGetContextPath() + '/members/' + arr.join(','),	// 배열 요소 사이에 , 삽입하고 문자열로 저장
		dataType: 'json',
		success: (resData)=>{		// resData {"deleteCount": 3}
