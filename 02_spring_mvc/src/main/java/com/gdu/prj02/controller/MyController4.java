...
public class MyController4 {
  ...
  // method=RequestMethod.GET 생략 가능, 디폴트는 GET
  // method 가 생략되면 value= 도 생략 가능
  @RequestMapping("/blog/list.do")
  public String list(Model model) {
    
    // DB 에서 select 한 결과라고 생각
  ...
  // 커맨드 객체를 이용한 요청 파라미터는 자동으로 model 에 저장된다.
  //@RequestMapping(value="/blog/add.do", method=RequestMethod.POST)
  public String add(BlogDto blog) { // 커맨드 객체의 Model 지정 방식 : 클래스 타입을 camelCase 로 변경해서 저장한다.(BlogDto -> blogDto 로 변경해서 저장)
  ...
  @RequestMapping(value="/blog/add.do", method=RequestMethod.POST)
  public String add2(@ModelAttribute("blog")
  BlogDto blog) {  // @ModelAttribute : 커맨드 객체가 Model 에 저장되는 이름을 지정할 때 사용한다.
